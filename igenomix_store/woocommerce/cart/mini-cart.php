		...
		?>
				...
				<div class="woocommerce-mini-cart-item <?php echo esc_attr(apply_filters('woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key)); ?> asidecart__cartitem cartitem">
					...
					<p class="quantity cartitem__quantity">
						...
						?>
						<?php if ($prodPrice && $prodPrice > 0) : ?>
							<?php echo sprintf('%s &times; %s%s', $cart_item['quantity'], price_fmt($prodPrice), $currency_symbol); ?>
						<?php else : ?>
							<?php echo sprintf('%s &times; %s', $cart_item['quantity'], __('Цена по запросу')); ?>
						<?php endif; ?>
					</p>
					...
					<button class="cartitem__remove">
						...
						<a href="<?php esc_url(wc_get_cart_remove_url($cart_item_key)); ?>" class="remove_from_cart_button" aria-label="<?php esc_attr__('Remove this item', 'woocommerce'); ?>" data-product_id="<?php echo esc_attr($product_id); ?>" data-cart_item_key="<?php echo esc_attr($cart_item_key); ?>" data-product_sku="<?php echo esc_attr($_product->get_sku()); ?>">
